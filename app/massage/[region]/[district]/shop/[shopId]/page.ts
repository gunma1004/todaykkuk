...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const charSum = (locationPrefix + shop.name + shopId + "todaykkuk_district_shop_seo_1000").split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  ...
  return {
    ...
    openGraph: {
      ...
      description: formattedDesc,
      url: `https://todaykkuk.netlify.app/massage/${region}/${encodeURIComponent(safeDecode(district))}/shop/${shopId}`,
      siteName: "투데이쿡",
      locale: "ko_KR",
      type: "website",
    },
